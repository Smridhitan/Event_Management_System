...
CREATE TRIGGER trg_handle_resource_update
...
BEGIN
    ...
    SELECT resource_type, available_quantity INTO r_type, available 
    FROM Resource WHERE resource_id = NEW.resource_id;

    -- Scenario A: Releasing an allocated resource
    IF NEW.usage_completed = TRUE AND OLD.usage_completed = FALSE THEN
        IF r_type = 'Furniture' THEN
            UPDATE Resource 
            SET available_quantity = available_quantity + OLD.quantity_allocated 
            WHERE resource_id = NEW.resource_id;
        END IF;
        SET NEW.allocation_status = 'Released';
        
    -- Scenario B: Incrementing an actively allocated resource
    ELSEIF NEW.usage_completed = FALSE AND OLD.usage_completed = FALSE AND NEW.quantity_allocated > OLD.quantity_allocated THEN
        IF (NEW.quantity_allocated - OLD.quantity_allocated) > available THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Insufficient resource quantity';
        END IF;
        UPDATE Resource 
        SET available_quantity = available_quantity - (NEW.quantity_allocated - OLD.quantity_allocated) 
        WHERE resource_id = NEW.resource_id;

    -- Scenario C: Re-allocating a previously released resource
    ELSEIF NEW.usage_completed = FALSE AND OLD.usage_completed = TRUE THEN
        IF NEW.quantity_allocated > available THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Insufficient resource quantity';
        END IF;
        UPDATE Resource 
